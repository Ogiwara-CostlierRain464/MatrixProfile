#ifndef MATRIXPROFILE_FFT_H
#define MATRIXPROFILE_FFT_H
...
void from_double_vec(const std::vector<double> &vec,
                     CArray &out_carr){
  out_carr = CArray(vec.size());
  for(size_t i = 0; i < vec.size(); i++){
    out_carr[i] = Complex(vec[i]);
  }
}
